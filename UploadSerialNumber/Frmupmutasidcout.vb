...
Public Class Frmupmutasidcout
    ...
    Dim minor, maxst, kdtk, nilaigrid, sts, npersent, jumka, iddcpnrm As Integer
    Dim kdprd As String
    ...
    Private Sub loaddata()
        ...
        MyAdapter = New System.Data.OleDb.OleDbDataAdapter("Select RTRIM(LTRIM(nomormutasi)),RTRIM(LTRIM(Kodeproduk)),RTRIM(LTRIM(Namapanjang)),RTRIM(LTRIM(nomorsn)),'' as CekStatus,'' as ceksn from [SheetMutasiout$]", MyConnection)
    ...
    Private Sub btnProses_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnProses.Click
        ...
        Dim sql1 As String
        Dim sql22 As String
        sql1 = "select * from SMITblSnMutasiOutDCTmp where nomutasi='" & txtnoto.Text & "' and iduser='" & StrNamaUser & "'"
        ...
        If Not RsConn.EOF() Then
            ...
            sql1 = "exec spSnUploadMutasioutdc 'insertMutasioutdc','" & txtnoto.Text & "','','',0,'" & StrNamaUser & "',''"
            RsConn = Conn.Execute(sql1)

            'If TextBox1.Text = "MUTASI GS TO BS" Then
            sql22 = "exec spSnUploadMutasioutdc 'updateMutasioutdc','" & txtnoto.Text & "','','',0,'" & StrNamaUser & "',''"
            RsConn = Conn.Execute(sql22)
            'End If


            MsgBox("Upload Data Sukses..")
            sql = "delete from SMITblSnMutasiOutDCTmp where nomutasi='" & txtnoto.Text & "' and iduser='" & StrNamaUser & "'"
            RsConn = Conn.Execute(sql)
            Call buka_new()
        Else
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Label3.Text = "UPLOAD SERIAL NUMBER MUTASI DC OUT"
        Call buka_new()
    End Sub
    ...
    Private Sub cekdata()
        ...
        For Each row As DataGridViewRow In Me.dgminmax.Rows
            ...
            Else
                sql = "SELECT * from MutasiDCDetailOut WHERE noMutasi=" & strnomutasi & " and idproduk in(select idproduk from mstproduk where kodeproduk='" & strkdproduk & "' and idsnqr=1)"
                RsConn = Conn.Execute(sql)
                If Not RsConn.EOF() Then
            ...
            Else
                ...
                If row.Cells.Item(5).Value = "SN OK" And row.Cells.Item(4).Value = "SKU OK" Then
                    ...
                    sql1 = "insert into SMITblSnMutasiOutDCTmp values(" & IdDC & ",'" & strnomutasi & "',(select idproduk from mstproduk where kodeproduk ='" & strkdproduk & "'),'" & nomorsn & "','" & StrNamaUser & "',getdate(),null," & iddcpnrm & ")"
    ...
    Private Sub BtnValidasi_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnValidasi.Click
        sql = "SELECT * from SMITblSnMutasiOutDCTmp where iduser='" & StrNamaUser & "' and nomutasi='" & txtnoto.Text & "'"
        'sql = "exec spSnUploadto 'Cektmp','" & txtnoto.Text & "',0,'',0,'" & StrNamaUser & "'"
        RsConn = Conn.Execute(sql)
        'Dim sntemp As Integer
        If Not RsConn.EOF Then
            sql = "delete from SMITblSnMutasiOutDCTmp where iduser='" & StrNamaUser & "' and nomutasi='" & txtnoto.Text & "'"
            'sql = "exec spSnUploadto 'hapustmp','" & txtnoto.Text & "',0,'',0,'" & StrNamaUser & "'"
            RsConn = Conn.Execute(sql)
            flagproses = True
            pbload.Visible = True
        ...
        Else
            sql = "SELECT * from SMITblSnMutasiOutDCTmp where nomutasi='" & txtnoto.Text & "'"
            'sql = "exec spSnUploadto 'Ceknoto','" & txtnoto.Text & "',0,'',0,'" & StrNamaUser & "'"
            RsConn = Conn.Execute(sql)
            'Dim sntemp As Integer
            If Not RsConn.EOF Then
                sql = "SELECT * from SMITblSnMutasiOutDCTmp where nomutasi='" & txtnoto.Text & "' and iduser<>'" & StrNamaUser & "'"
                RsConn = Conn.Execute(sql)
                If Not RsConn.EOF Then
    ...
    Private Sub cekqty()
        sql = "SELECT count(nomorsn) as jmlsn from SMITblSnMutasiOutDCTmp where iduser='" & StrNamaUser & "' and nomutasi='" & txtnoto.Text & "'"
        ...
        sql = "SELECT count(distinct(idproduk)) as jmlsku from SMITblSnMutasiOutDCTmp where iduser='" & StrNamaUser & "' and nomutasi='" & txtnoto.Text & "'"
    ...
    Sub tampilkansku()
        ...
        strsql = "SELECT a.nomutasi,b.namadc as dcpengirim,c.namadc as dcpenerima,d.kodeproduk,d.namapanjang,a.qty,a.iddcpenerima from MutasiDCDetailOut a JOIN MstDC b on a.idDcPengirim=b.iddc JOIN MstDC c on a.idDcPenerima=c.iddc JOIN MstProduk d on a.idproduk=d.idproduk WHERE a.idproduk in(SELECT idProduk from MstProduk WHERE idsnQr=1) and a.iddcpengirim=" & IdDC & "and a.nomutasi='" & txtnoto.Text & "'"
    ...
    Sub hitungskuTO()
        ...
        sql = "SELECT count(a.nomutasi) as jumlahitem from MutasiDCDetailOut a JOIN MstProduk b on a.idproduk=b.idproduk WHERE b.idsnqr=1 and a.nomutasi='" & txtnoto.Text & "' and a.iddcpengirim=" & IdDC & ""
        RsConn = Conn.Execute(sql)
        ...
        sql = "SELECT isnull(sum(a.qty),0) as Jumlahqty from MutasiDCDetailOut a JOIN MstProduk b on a.idproduk=b.idproduk WHERE b.idsnqr=1 and a.nomutasi='" & txtnoto.Text & "' and a.iddcpengirim=" & IdDC & ""
    ...
    Private Sub Button5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button5.Click
        ...
        txtnoto.Text = FrmFind.cari("Mutasidcout")
    ...
    End Sub
    Sub cektodipakai()
        sql = "SELECT * from SMITblSnMutasiOutDCTmp where nomutasi='" & txtnoto.Text & "' and iduser<>'" & lbnama.Text & "' and iddcpengirim=" & IdDC & ""
    ...
    Sub cektotemp()
        ...
        If Not RsConn.EOF Then
            ...
            RsConn = Conn.Execute(sql)
            MsgBox("Cancel Validasi Mutasi dc Out Berhasil...")
            Call buka_new()
        Else
            MsgBox("Nomor Mutasi dc Out Validasi Tidak Ditemukan...")
        End If
    End Sub
